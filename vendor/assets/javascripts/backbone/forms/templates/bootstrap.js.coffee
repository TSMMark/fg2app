...
((Form) ->
  ...
  ###
  Form.template = _.template("<form class=\"form-horizontal\" data-fieldsets></form>")
  Form.Fieldset.template = _.template("<fieldset data-fields> <% if (legend) { %> <legend><%= legend %></legend> <% } %> </fieldset>")
  Form.Field.template = _.template("<div class=\"control-group field-<%= key %>\"> <label class=\"control-label\" for=\"<%= editorId %>\"><%= title %></label> <div class=\"controls\"> <span data-editor></span> <div class=\"help-inline\" data-error></div> <div class=\"help-block\"><%= help %></div> </div> </div>")
